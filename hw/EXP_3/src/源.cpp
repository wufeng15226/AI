...
{
	int value = 0, my_front_tiles = 0, opp_front_tiles = 0, my_tiles = 0, opp_tiles = 0;
	int i, j, k, x, y;
		...
		for (j = 0; j < 6; j++)
		{
			...
			if (board->cell[i][j].color)
			{
				for (k = 0; k < 8; k++)
				{
					x = i + X1[k];
					y = j + Y1[k];
					if (x >= 0 && x < 6 && y >= 0 && y < 6 && board->cell[i][j].color == 0)
					{
						if (board->cell[i][j].color == player)
							my_front_tiles++;
						else
	...
	l = -25 * (my_tiles - opp_tiles);

	return (int)(p + f * 7 + c * 80 + l * 40);
}
...
Do *MyFind(Othello *board, enum Option player, int step, int min, int max, Do *choice)
{
	...
	if (num == 0)
	{
		if (board->Rule(board, (enum Option) - player))
		{
			...
			pNextChoice = MyFind(&tempBoard, (enum Option) - player, step - 1, -max, -min, pNextChoice);
	...
	k = 0;
	for (i = 0; i < 6; i++) //这里粗糙地直接处理，没有像AI那样分组处理，可能会慢一点（step=10情况下变化不大），但不影响结果
	{
		for (j = 0; j < 6; j++)
		{
	...
	for (k = 0; k < num; k++) //永远求最大值，最小值的情况通过取负解决
	{
		...
		if (thisChoice.score > min && thisChoice.score < max) //接下来的分支中可能还有值在thisChoice.score和max之间，所以需要继续搜索
		{													  //只有thisChoice.score>min时才需要更新min
		...
		else if (thisChoice.score >= max) //上层的min肯定不会选择当前的节点，所以剩余的分支都可以减掉
		{
			choice->score = thisChoice.score;
			choice->pos.first = thisChoice.pos.first;
			choice->pos.second = thisChoice.pos.second;
