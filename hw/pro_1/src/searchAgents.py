...
def cornersHeuristic(state, problem):
    ...
    #     if state[1][i] == 0:
    #         if flag:
    #             sum += abs(x-corners[i][0])+abs(y-corners[i][1])
    #             flag = False
    #         else:
    #             sum = (sum+3*abs(x-corners[i][0])+abs(y-corners[i][1]))/4
    #         cnt += 1
    # return sum*cnt
    # v1 依次寻找最小曼哈顿距离
    sum = 0
...
def foodHeuristic(state, problem):
    ...
    # for i in food:
    #     min = 99999
    #     dis = abs(position[0] - i[0]) + abs(position[1] - i[1])
    #     if min > dis:
    #         min = dis
    #         nx, ny = i
    #     sum += min
