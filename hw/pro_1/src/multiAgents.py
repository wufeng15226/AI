...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def MinMaxDFS(self, gameState, currDepth, agentIndex):
        ...
        scores = []
        for i in succ:
            scores.append(self.MinMaxDFS(gameState.generateSuccessor(agentIndex, i),currDepth,agentIndex+1))
        if not agentIndex:
            mmax = max(scores)
            if currDepth == 1:
                bestIndices = []
    ...
    def AlphaBetaDFS(self, gameState, currDepth, agentIndex,MIN,MAX):
        ...
        if not agentIndex:
            if currDepth == 1:
                scores = []
                for i in succ:
                    scores.append(self.AlphaBetaDFS(gameState.generateSuccessor(agentIndex,i),currDepth,agentIndex+1,MIN,MAX))
                MMAX = max(scores)
                bestIndices = []
                for i in range(len(scores)):
                    if scores[i] == MMAX:
            ...
            MMAX = -99999
            for i in succ:
                MMAX = max(MMAX,self.AlphaBetaDFS(gameState.generateSuccessor(agentIndex,i),currDepth,agentIndex+1,MIN,MAX))
                if MAX <= MMAX: #剪枝
                    return MMAX
                MAX = max(MAX,MMAX)
            return MMAX
            ...
            MMIN = 99999
            for i in succ:
                MMIN = min(MMIN,self.AlphaBetaDFS(gameState.generateSuccessor(agentIndex,i),currDepth,agentIndex+1,MIN,MAX))
                if MIN >= MMIN: #剪枝
                    return MMIN
                MAX = min(MAX,MMIN)
            return MMIN
